...
public class FixturesNodeModel extends AbstractFootballQueryNodeModel {
    ...
    static final String CFGKEY_INCREMENTAL_VARIABLE = "incrementalVariable";
    ...
    static final String MODE_INCREMENTAL = "incremental";
    ...
    private void validateDateMode() throws InvalidSettingsException {
        ...
        } else if (MODE_INCREMENTAL.equals(mode)) {
            if (m_incrementalVariable.getStringValue().isEmpty()) {
                throw new InvalidSettingsException("Please specify a flow variable name for incremental mode");
            }
        }
    }
    ...
    private String[] resolveDateRange() throws InvalidSettingsException {
        ...
        if (MODE_RANGE.equals(mode)) {
            ...
            // Incremental mode: Query from last run date to today
            String varName = m_incrementalVariable.getStringValue();
            try {
                FlowVariable flowVar = getAvailableFlowVariables().get(varName);
                if (flowVar == null) {
                    // First run: use last 7 days as default
                    getLogger().info("Incremental variable '" + varName + "' not found. Using default: last 7 days");
                    return new String[]{today.minusDays(7).toString(), today.toString()};
                }
                String lastRunDate = flowVar.getStringValue();
                return new String[]{lastRunDate, today.toString()};
            } catch (Exception e) {
                throw new InvalidSettingsException(
                    "Failed to read incremental flow variable '" + varName + "': " + e.getMessage());
            }
        }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        ...
        m_incrementalVariable.saveSettingsTo(settings);
    }
    ...
    protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
        ...
        // DateRangePanel settings - backward compatibility
        if (settings.containsKey(CFGKEY_DATE_MODE)) {
            ...
            m_incrementalVariable.validateSettings(settings);
        }
        // fromDate/toDate always exist for backward compatibility
    ...
    protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
        ...
        if (settings.containsKey(CFGKEY_DATE_MODE)) {
            ...
            m_incrementalVariable.loadSettingsFrom(settings);
        } else {
            // Old workflow: use default "range" mode
            m_dateMode.setStringValue(MODE_RANGE);
        }
        // fromDate/toDate always load for backward compatibility
